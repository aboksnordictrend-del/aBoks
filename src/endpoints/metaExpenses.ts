// Admin-only Meta Ads detail data, registered as GET
// /api/admin/integrations/meta/expenses?since&until. Returns only imported (source =
// 'meta-api', channel = 'meta') day records for the period, plus a computed summary. Manual
// records are never included. No secrets in the response.
...
import { validateSyncDates, SyncValidationError } from '@/lib/meta/sync'
...
export const metaExpensesEndpoint: Endpoint = {
  path: '/admin/integrations/meta/expenses',
  method: 'get',
  handler: async (req: PayloadRequest): Promise<Response> => {
    ...
    try {
      ...
      const anyImported = await req.payload.find({
        collection: 'marketing-expenses',
        where: { and: [{ channel: { equals: 'meta' } }, { source: { equals: 'meta-api' } }] },
        depth: 0,
        limit: 1,
        pagination: true,
        overrideAccess: false,
      ...
      return Response.json(
        {
          ...
          rows,
          summary: metaExpensesSummary(rows),
      ...
      req.payload.logger.error(
        `[marketing] meta expenses failed: ${err instanceof Error ? err.message : 'unknown'}`,
      )
      return Response.json({ error: 'Kunne ikke hente Meta-kostnader.' }, { status: 500 })
    }
  },
}
