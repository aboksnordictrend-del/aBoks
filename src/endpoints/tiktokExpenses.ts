// Admin-only TikTok Ads detail data, registered as GET
// /api/admin/integrations/tiktok/expenses?since&until. Returns only imported (source =
// 'tiktok-ads', channel = 'tiktok') day records for the period, plus a computed summary.
// Manual records are never included. No secrets in the response.
//
// Mirrors src/endpoints/pinterestExpenses.ts field for field.
...
export const tiktokExpensesEndpoint: Endpoint = {
  path: '/admin/integrations/tiktok/expenses',
  method: 'get',
  handler: async (req: PayloadRequest): Promise<Response> => {
    ...
    try {
      ...
      ;({ since, until } = validateSyncDates({
        since: str(query, 'since'),
        until: str(query, 'until'),
      }))
    } catch (err) {
      const message = err instanceof TikTokSyncValidationError ? err.message : 'Ugyldig forespørsel.'
    ...
    try {
      ...
      req.payload.logger.error(
        `[marketing] tiktok expenses failed: ${err instanceof Error ? err.message : 'unknown'}`,
      )
      return Response.json({ error: 'Kunne ikke hente TikTok Ads-kostnader.' }, { status: 500 })
    }
  },
}
