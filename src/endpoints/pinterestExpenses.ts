// Admin-only Pinterest Ads detail data, registered as GET
// /api/admin/integrations/pinterest/expenses?since&until. Returns only imported (source =
// 'pinterest-ads', channel = 'pinterest') day records for the period, plus a computed summary.
// Manual records are never included. No secrets in the response.
//
// Mirrors src/endpoints/googleExpenses.ts field for field.
...
export const pinterestExpensesEndpoint: Endpoint = {
  path: '/admin/integrations/pinterest/expenses',
  method: 'get',
  handler: async (req: PayloadRequest): Promise<Response> => {
    ...
    try {
      ...
      req.payload.logger.error(
        `[marketing] pinterest expenses failed: ${err instanceof Error ? err.message : 'unknown'}`,
      )
      return Response.json({ error: 'Kunne ikke hente Pinterest Ads-kostnader.' }, { status: 500 })
    }
  },
}
