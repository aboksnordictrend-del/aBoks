...
// Sync actions for Meta Ads. The user never picks dates here: the server decides the
// window from the mode (incremental = trailing 14-day overlap, full = entire history).
// Viewing periods are a separate concern, owned by the page filter and by Analyse.
...
export default function MetaSyncButton({
  ...
  const runSync = async (mode: MetaSyncMode) => {
    setPhase('submitting')
    setError('')
    setResult(null)

    try {
      const res = await fetch(MARKETING_API.metaSync, {
        method: 'POST',
        credentials: 'include',
  ...
  return (
    <div className={styles.wrap}>
      ...
      >
        {submitting ? 'Synkroniserer …' : hasData ? 'Oppdater' : 'Synkroniser'}
      </button>
      ...
      {dialogOpen && (
        <div className={styles.backdrop} onMouseDown={close}>
          ...
          >
            {phase === 'confirmFull' && (
              <>
                <h2 id="meta-sync-title" className={styles.title}>
                  Full synkronisering
                </h2>
                <p className={styles.hint}>
                  Full synkronisering henter hele den tilgjengelige historikken på nytt.
                  Eksisterende Meta API-poster oppdateres uten å opprette duplikater.
                </p>
            ...
            {phase === 'submitting' && (
              <>
                ...
                <p className={styles.hint}>Henter data fra Meta. Dette kan ta litt tid.</p>
              </>
            )}
            ...
            {phase === 'conflict' && result && (
              <>
                ...
                <div className={styles.result} role="alert">
                  <p>
                    Det finnes manuelle Meta Ads-kostnader som overlapper perioden. Fjern eller
