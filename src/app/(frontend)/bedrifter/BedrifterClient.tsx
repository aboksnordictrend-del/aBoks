...
import { buildPriceTableRows, quoteThresholdFor, type PriceTableRow } from '@/lib/quantityPricing'
import {
  QUOTE_TEXT,
  quoteContextFor,
  quoteRequestMessage,
...
interface ProductSection {
  ...
  /**
   * The product's quantity price bands, generated by the shared pricing engine from its live
   * CMS price. There is deliberately no B2B price table written into this page — every figure
   * below comes from the same table the cart and the checkout charge from, so a price can
   * never be advertised here that the shop does not actually apply.
   *
   * A product with no configured bands gets a single open-ended row, which is the truth for
   * it, and no quote row.
   */
  priceTable: PriceTableRow[]
  /** The quantity the quote row's CTA asks about — the threshold itself. */
...
/**
 * One product's quantity prices.
 *
 * Every row is generated — see `BedrifterProduct.priceTable`. The last row is the quote row:
 * it carries the same unit price as the row above it, because reaching the quote threshold is
 * not a further reduction, and offers the enquiry instead of repeating the amount.
 */
function PriceTable({
  rows,
  productName,
  ...
  rows: PriceTableRow[]
  productName: string
  /** Presets the form with this product and the quote quantity, then scrolls to it. */
  onQuoteRequest: React.MouseEventHandler<HTMLAnchorElement>
}) {
  // A single open-ended row is a product with no quantity pricing — there is no table to show.
  if (rows.length < 2) return null
  // A product whose price could not be read would print a table of «kr 0». Better nothing.
  if (rows.some((row) => !(row.unitPrice > 0))) return null

  return (
    <div style={{ width: '100%', margin: '0 0 30px' }}>
      <p style={cardLabelStyle}>Pris per stk.</p>
      <table
        style={{
          width: '100%',
      ...
      >
        <caption className="sr-only">Mengdepriser for {productName}, eks. frakt</caption>
        <thead>
          <tr>
            <th
              scope="col"
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.range} style={{ borderTop: `1px solid ${BORDER_WARM}99` }}>
              <th
                scope="row"
                style={{ textAlign: 'left', padding: '11px 0', fontWeight: 600, color: INK }}
              >
                {row.range}
              </th>
              <td style={{ textAlign: 'right', padding: '11px 0', color: INK }}>
                {row.isQuoteRow ? (
                  // The published price still applies here — the link is an addition, not a
                  // replacement, and the amount is repeated so nothing looks withheld.
                  <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: '10px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    <span style={{ color: MUTED }}>{formatPrice(row.unitPrice)}</span>
                    <a
                      href="#tilbud"
                      data-btn
                  ...
                  <span style={{ fontWeight: 600 }}>{formatPrice(row.unitPrice)}</span>
                )}
              </td>
            </tr>
          ))}
      ...
      >
        Prisene er inkl. mva. og gjelder per produkt — antall av ulike produkter legges ikke
        sammen. Prisen gjelder alle enhetene i bestillingen.
      </p>
    </div>
  )
}
...
function ProductSectionBlock({
  ...
  /** The price table's quote row — presets the product and the quote quantity too. */
  onPriceTableQuote: React.MouseEventHandler<HTMLAnchorElement>
}) {
  const image = (
    <Image
      src={section.image}
    ...
    >
      ...
      <div className={`contents md:flex md:flex-col ${imageFirst ? 'md:order-2' : 'md:order-1'}`}>
        ...
        >
          ...
          <PriceTable
            rows={section.priceTable}
            productName={section.name}
            onQuoteRequest={onPriceTableQuote}
          />
...
export default function BedrifterClient({ products }: { products: BedrifterProduct[] }) {
   ...
   */
  const productSections: (ProductSection & { product: BedrifterProduct })[] = [
    ...products.map((product) => {
      ...
      return {
        product,
        // Generated from the shared engine, never restated here. The quote row's quantity is
        // the threshold itself, so the enquiry it opens asks about the quantity it names.
        priceTable: buildPriceTableRows(product),
        quoteQuantity: quoteThresholdFor(product),
        name: product.title,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(64px,8vw,112px)' }}>
            {productSections.map((section, index) => (
              <ProductSectionBlock
                ...
                onPriceTableQuote={requestProductQuote(
                  section.product,
